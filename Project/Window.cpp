...
const char *pixelShader = R"(
    ...
    void main() {
        ...
        int block_size = 400 / 8;
        vec4 pColorWhite = vec4(1, 1, 1, 1);          // r, g, b, alpha
        vec4 pColorBlack = vec4(0, 0, 0, 1);
        
        int row =  int(gl_FragCoord.x / block_size);
        int column =  int(gl_FragCoord.y / block_size);
...
int main() {                                                // application entry
    ...
    GLFWwindow *w = glfwCreateWindow(400, 400, "Clear to Red", NULL, NULL);
    if (!w)
